...
async function authenticate(status, persona, password) {
...
async function run() {
  ...
  // Exists only for the current local run; it is never written to disk or reports.
  ...
  await mapWithConcurrency(personas, 10, async (persona) => {
    const { error } = await admin.auth.admin.createUser({ email: persona.email, password, email_confirm: true, user_metadata: { synthetic: true, locale: persona.locale, activity: persona.activity } });
  ...
  const authenticated = await mapWithConcurrency(personas, 10, async (persona) => {
    try { return await authenticate(status, persona, password); }
