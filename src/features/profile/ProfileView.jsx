...
export default function ProfileView({ locale = 'ko', cards, categories, savedPostIds, profile, profileLoading, profileNotice, isAuthenticated, blockedMembers = [], onCheckHandle, onLoadHandleSuggestions, onSaveHandle, onDelete, onRemoveScrap, onUpload, onUnblock, onSignOut }) {
  ...
  return <section className="editorial-profile w-full pb-3 pt-1">
    ...
    {isAuthenticated && <section className="profile-account-actions mt-7 border-t border-surface-container-high pt-3"><div className="mb-3"><div className="flex items-center justify-between"><h2 className="text-xs font-bold text-white">{locale === 'en' ? 'Blocked accounts' : '차단한 계정'}</h2><span className="font-mono text-[10px] text-slate-400">{blockedMembers.length}</span></div>{blockedMembers.length ? <div className="mt-2 flex flex-col gap-1.5">{blockedMembers.map((member) => <div key={member.id} className="flex items-center gap-2 rounded-md bg-surface-container-low px-2.5 py-2"><span className="material-symbols-outlined text-sm text-slate-400">person_off</span><span className="min-w-0 flex-1 truncate text-[11px] text-slate-200">{member.author ? `@${member.author}` : (locale === 'en' ? 'Blocked account' : '차단한 계정')}</span><button type="button" onClick={() => onUnblock(member.id)} className="rounded border border-[#c4c6cd] px-2 py-1 text-[10px] font-bold text-slate-200">{locale === 'en' ? 'Unblock' : '차단 해제'}</button></div>)}</div> : <p className="mt-1 text-[10px] text-slate-500">{locale === 'en' ? 'No blocked accounts.' : '차단한 계정이 없습니다.'}</p>}</div><button type="button" onClick={onSignOut} className="profile-account-actions__sign-out">{locale === 'en' ? 'Sign out' : '로그아웃'}</button></section>}
  </section>;
}
