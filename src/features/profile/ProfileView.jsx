...
export default function ProfileView({ locale = 'ko', cards, categories, savedPostIds, profile, profileLoading, profileNotice, isAuthenticated, blockedMembers = [], onCheckHandle, onLoadHandleSuggestions, onSaveHandle, onDelete, onRemoveScrap, onUpload, onUnblock, onSignOut }) {
  ...
  return <section className="editorial-profile w-full pb-3 pt-1">
    ...
    <SurfaceCard as="article" className="mb-4 p-4"><div className="flex items-center gap-3.5"><div className="relative"><img src={cards[0]?.imageUrl} alt="내 프로필" className="media-protected h-14 w-14 rounded-full border-2 border-[#c5a059] object-cover" draggable="false" onContextMenu={(event) => event.preventDefault()} onDragStart={(event) => event.preventDefault()} /><span className="absolute bottom-0 right-0 flex h-3.5 w-3.5 items-center justify-center rounded-full border-2 border-background bg-[#c5a059]"><span className="material-symbols-outlined text-[8px] font-bold text-white">check</span></span></div><div className="flex-1"><h1 className="flex flex-wrap items-center gap-1.5 font-headline text-lg font-bold text-white">{displayHandle ? `@${displayHandle}` : (locale === 'en' ? 'Set your public ID' : '공개 아이디 설정 필요')} <span className="material-symbols-outlined text-sm text-cyan-glow">verified</span>{isAuthenticated && handleReady && !editingHandle && <button type="button" onClick={() => setEditingHandle(true)} className="rounded border border-[#c52a52]/55 px-1.5 py-0.5 text-[9px] font-bold text-[#f487a3] hover:bg-[#c52a52]/10">{locale === 'en' ? 'Change' : '변경'}</button>}</h1><p className="text-[11px] text-slate-400">오늘의 룩과 일상의 순간을 기록하고 있어요.</p><div className="mt-1 flex items-center gap-1.5"><span className="rounded border border-[#c5a059]/50 bg-[#f9f7f2] px-1.5 py-0.5 font-mono text-[9px] text-cyan-glow">CURATOR</span><span className="font-mono text-[10px] text-slate-400">FACt.Smack Score: <strong className="text-white">94.6</strong></span></div></div></div><div className="mt-3 grid grid-cols-3 gap-2 border-t border-surface-container-high pt-3"><Metric value={mine.length} label="내 업로드" /><Metric value={votes.toLocaleString()} label="받은 투표" color="text-cyan-glow" /><Metric value={`${approval}%`} label="평균 호감도" color="text-[#735c00]" /></div></SurfaceCard>
