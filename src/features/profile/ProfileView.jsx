...
export default function ProfileView({ locale = 'ko', cards, categories, savedPostIds, profile, profileLoading, profileNotice, isAuthenticated, blockedMembers = [], onCheckHandle, onLoadHandleSuggestions, onSaveHandle, onDelete, onRemoveScrap, onUpload, onUnblock, onSignOut }) {
  ...
  return <section className="editorial-profile w-full pb-3 pt-1">
    ...
    {isAuthenticated && !profileLoading && handleReady && !editingHandle && <div className="mb-3 flex items-center justify-between gap-3 rounded-xl border border-surface-container-high bg-surface-container-low px-3 py-2.5"><div className="min-w-0"><p className="text-[10px] font-semibold uppercase tracking-wider text-slate-400">{locale === 'en' ? 'Public ID' : '공개 아이디'}</p><p className="truncate font-mono text-sm font-bold text-white">@{displayHandle}</p></div><button type="button" onClick={() => setEditingHandle(true)} className="shrink-0 rounded-md border border-[#c52a52]/60 px-2.5 py-1.5 text-[11px] font-bold text-[#f487a3] hover:bg-[#c52a52]/10">{locale === 'en' ? 'Change' : '변경'}</button></div>}
