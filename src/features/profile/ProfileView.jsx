...
import { canSubmitHandle, getPublicHandle, normalizeHandle, PUBLIC_HANDLE_MAX_LENGTH } from '../../services/profileService.js';
import { sortPostsNewestFirst } from './profileOrdering.js';

/** 정의: 개인 업로드 요약, 투표 지표, 내 게시물 관리 행동을 제공하는 프로필 화면이다. */
export default function ProfileView({ locale = 'ko', cards, profileCards, scrapCards, categories, savedPostIds, profile, profileLoading, profileNotice, isAuthenticated, canModerate = false, blockedMembers = [], onCheckHandle, onLoadHandleSuggestions, onSaveHandle, onDelete, onRemoveScrap, onOpenScrap, onUpload, onOpenModeration, onUnblock, onSignOut }) {
  const [editingHandle, setEditingHandle] = useState(false);
  // An authenticated profile must never fall back to feed fixtures. The private
  // server library is the only authority for what its owner may manage here.
  const profileLibrary = profileCards ?? (isAuthenticated ? [] : cards);
  const mine = sortPostsNewestFirst(profileLibrary.filter((card) => card.isMyUpload));
  ...
  const profileDescription = locale === 'en'
    ? "Documenting today's looks and everyday moments."
    : '오늘의 룩과 일상의 순간을 기록하고 있어요.';
  ...
  return <section className="editorial-profile w-full pb-3 pt-1">
    ...
    <SurfaceCard as="article" className="profile-summary mb-4 p-4"><div className="profile-summary__head"><div className="profile-summary__avatar relative"><span className="profile-summary__avatar-fallback material-symbols-outlined" aria-hidden="true">person</span>{mine[0]?.imageUrl && <img src={mine[0].imageUrl} alt="" className="media-protected h-14 w-14 rounded-full border-2 border-[#c5a059] object-cover" draggable="false" onError={(event) => { event.currentTarget.hidden = true; }} onContextMenu={(event) => event.preventDefault()} onDragStart={(event) => event.preventDefault()} />}<span className="absolute bottom-0 right-0 flex h-3.5 w-3.5 items-center justify-center rounded-full border-2 border-background bg-[#c5a059]"><span className="material-symbols-outlined text-[8px] font-bold text-white">check</span></span></div><div className="profile-summary__content"><div className="profile-summary__identity"><h1 className="min-w-0 flex-1 font-headline text-lg font-bold text-white" title={displayHandle ? `@${displayHandle}` : undefined}><span className="profile-summary__handle">{displayHandle ? `@${displayHandle}` : (locale === 'en' ? 'Set your public ID' : '공개 아이디 설정 필요')}</span></h1>{isAuthenticated && handleReady && !editingHandle && <button type="button" onPointerDown={isolateProfileTouch} onPointerUp={beginHandleEditFromTouch} onPointerCancel={isolateProfileTouch} onClick={beginHandleEdit} className="profile-summary__change min-h-9 shrink-0 rounded-md border border-[#c52a52]/55 px-3 text-xs font-bold text-[#c52a52] hover:bg-[#c52a52]/10">{locale === 'en' ? 'Change' : '변경'}</button>}</div><p className="profile-summary__description text-[11px] text-slate-400">{profileDescription}</p><div className="profile-summary__score mt-1"><span className="rounded border border-[#c5a059]/50 bg-[#f9f7f2] px-1.5 py-0.5 font-mono text-[9px] text-cyan-glow">CURATOR</span><span className="font-mono text-[10px] text-slate-400">FACt.Smack Score: <strong className="text-white">94.6</strong></span></div></div></div><div className="mt-3 grid grid-cols-3 gap-2 border-t border-surface-container-high pt-3"><Metric value={mine.length} label={locale === 'en' ? 'My uploads' : '내 업로드'} /><Metric value={votes.toLocaleString()} label={locale === 'en' ? 'Ratings received' : '받은 투표'} color="text-cyan-glow" /><Metric value={`${approval}%`} label={locale === 'en' ? 'Average approval' : '평균 호감도'} color="text-[#735c00]" /></div></SurfaceCard>
    <div className="mb-2.5 flex items-center justify-between"><h2 data-no-translate className="font-headline text-sm font-bold text-white">{locale === 'en' ? 'My uploaded photo insights' : '내가 업로드한 사진 분석'}</h2><button data-no-translate type="button" onClick={onUpload} className="flex items-center gap-0.5 text-sm font-bold text-cyan-glow"><span className="material-symbols-outlined text-base" aria-hidden="true">add</span>{locale === 'en' ? 'New upload' : '새로 업로드'}</button></div>
...
function HandleSetup({ locale, profileId, initialHandle = '', isEditing = false, onCheck, onLoadSuggestions, onSave, onCancel }) {
  ...
  function focusHandleInput() {
    // Let the native input receive its own touch sequence. Deferring the focus
    // covers iOS browsers that drop the first focus after a profile redirect.
