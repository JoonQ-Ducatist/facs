...
function VisibilityChoice({ selected, icon, label, description, onSelect }) {
  return <button type="button" role="radio" aria-checked={selected} onClick={onSelect} className={`min-h-[76px] rounded-md border p-2.5 text-left transition-colors ${selected ? 'border-[#c5a059] bg-[#fbf9f4] shadow-[0_2px_8px_rgba(0,0,0,.05)]' : 'border-[#d4d0c8] bg-[#e8e8e6] hover:bg-[#eeeeec]'}`}><span className={`flex items-center gap-1.5 text-xs font-bold ${selected ? 'text-[#1b1c19]' : 'text-[#5d6065]'}`}><span className={`material-symbols-outlined text-[16px] ${selected ? 'text-cyan-glow' : 'text-[#74777d]'}`}>{icon}</span>{label}</span><span className={`mt-1 block text-[10px] leading-relaxed ${selected ? 'text-[#5b5e63]' : 'text-[#777a7f]'}`}>{description}</span></button>;
}
