...
export default function UploadView({ categories, locale = 'ko', publicHandle = '', onSubmit, onMessage }) {
  ...
  return <section className="editorial-upload w-full pb-3 pt-1">
    ...
    <form className="flex flex-col gap-3.5" onSubmit={submit}>
      ...
      <fieldset className="rounded-lg border border-surface-container-high bg-surface-container-low p-3"><legend className="px-1 text-[11px] font-bold uppercase tracking-wider text-slate-300">2. 공개 범위</legend><div className="grid grid-cols-2 gap-2"><label className={`cursor-pointer rounded-md border bg-[#172437]/90 p-2.5 transition-colors ${visibility === 'public' ? 'border-cyan-glow shadow-[inset_0_0_0_1px_rgba(0,240,255,.18)]' : 'border-white/20'}`}><input className="sr-only" type="radio" name="visibility" value="public" checked={visibility === 'public'} onChange={() => setVisibility('public')} /><span className="flex items-center gap-1.5 text-xs font-bold text-white"><span className="material-symbols-outlined text-[16px] text-cyan-glow">public</span>전체 공개</span><span className="mt-1 block text-[10px] leading-relaxed text-slate-300">피드에서 누구나 볼 수 있어요.</span></label><label className={`cursor-pointer rounded-md border bg-[#172437]/90 p-2.5 transition-colors ${visibility === 'followers' ? 'border-[#c5a059] shadow-[inset_0_0_0_1px_rgba(197,160,89,.2)]' : 'border-white/20'}`}><input className="sr-only" type="radio" name="visibility" value="followers" checked={visibility === 'followers'} onChange={() => setVisibility('followers')} /><span className="flex items-center gap-1.5 text-xs font-bold text-white"><span className="material-symbols-outlined text-[16px] text-[#f1d690]">group</span>팔로워만</span><span className="mt-1 block text-[10px] leading-relaxed text-slate-300">나와 나를 팔로우한 사람만 볼 수 있어요.</span></label></div></fieldset>
      ...
      <div><p className="mb-1 text-[11px] font-bold uppercase tracking-wider text-slate-300">{selectedTheme.evaluationType === 'NUMERIC_AGE' ? '6.' : '4.'} 게시 아이디</p><div className="flex items-center gap-2 rounded-xl border border-surface-container-high bg-surface-container px-3 py-2 text-xs text-white sm:text-sm"><span className="material-symbols-outlined text-[16px] text-cyan-glow">person</span><span className="font-semibold">@{publicHandle}</span><span className="ml-auto text-[10px] text-slate-400">프로필에서 변경</span></div></div>
