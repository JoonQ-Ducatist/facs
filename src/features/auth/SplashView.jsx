...
import { localeUrl } from '../../services/locale.js';
  ...
  return (
    <main className="splash-screen relative mx-auto h-full max-w-none overflow-hidden bg-[#051424] text-white shadow-2xl">
      ...
      <div className="relative z-10 flex h-full flex-col px-5 pb-6 pt-10">
        <button type="button" className="splash-language-toggle" onClick={() => { window.location.assign(localeUrl(locale === 'ko' ? 'en' : 'ko')); }} aria-label={locale === 'ko' ? '영어로 보기' : 'View in Korean'} title={locale === 'ko' ? 'English' : '한국어'}>{locale === 'ko' ? 'EN' : '한글'}</button>
