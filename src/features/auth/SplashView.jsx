          ...
          <div className="relative flex flex-col gap-2">
            ...
            {selectedProvider === 'email' && emailOpen ? (
              <div className="relative mx-auto w-full rounded-xl border border-[#ecd8a8]/70 bg-white/[0.14] p-3 shadow-inner">
                {emailSent ? (
                  <form className="flex flex-wrap gap-2" onSubmit={submitCode} aria-busy={isCodeVerifying}>
                    ...
                    <input required inputMode="numeric" autoComplete="one-time-code" maxLength={EMAIL_OTP_LENGTH} value={verificationCode} onChange={(event) => setVerificationCode(sanitizeEmailOtp(event.target.value))} placeholder={locale === 'en' ? `${EMAIL_OTP_LENGTH}-digit code` : `${EMAIL_OTP_LENGTH}자리 인증 코드`} className="h-10 min-w-[154px] flex-1 rounded-full border border-[#ecd8a8]/85 bg-black/15 px-3 text-center text-sm tracking-[0.2em] text-white placeholder:tracking-normal placeholder:text-white/45 outline-none focus:border-[#de3c65]" />
                    <button type="submit" disabled={isCodeVerifying || !isCompleteEmailOtp(verificationCode)} className="flex h-10 shrink-0 items-center justify-center rounded-full bg-[#c52a52] px-3.5 text-xs font-extrabold text-white transition duration-150 hover:bg-[#de3c65] active:scale-95 disabled:cursor-not-allowed disabled:opacity-55">{isCodeVerifying ? (locale === 'en' ? 'Checking...' : '확인 중...') : (locale === 'en' ? 'Verify' : '인증 완료')}</button>
                  ...
                  <form className="flex flex-wrap gap-2" onSubmit={submitEmail} aria-busy={isEmailSending}>
                    <input required disabled={isEmailSending} type="email" value={email} onChange={(event) => setEmail(event.target.value)} placeholder={locale === 'en' ? 'you@example.com' : '이메일 주소'} className="h-10 min-w-[154px] flex-1 rounded-full border border-[#ecd8a8]/85 bg-black/15 px-3 text-sm text-white placeholder:text-white/45 outline-none focus:border-[#de3c65] disabled:cursor-not-allowed disabled:opacity-55" />
                    <button type="submit" disabled={isEmailSending} className="flex h-10 shrink-0 items-center justify-center rounded-full bg-[#c52a52] px-3.5 text-xs font-extrabold text-white transition duration-150 hover:bg-[#de3c65] active:scale-95 active:bg-[#9f1f41] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#ecd8a8] disabled:cursor-not-allowed disabled:opacity-55">{isEmailSending && <span className="material-symbols-outlined mr-1 animate-spin text-[14px]" aria-hidden="true">progress_activity</span>}{isEmailSending ? (locale === 'en' ? 'Sending...' : '보내는 중...') : (locale === 'en' ? 'Send code' : '인증 코드 보내기')}</button>
                  </form>
                )}
