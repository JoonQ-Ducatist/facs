...
export default function FeedView({ locale = 'ko', categories, cards, card, currentIndex, activeCategory, hasVoted, canViewLiveReactions = false, liveReactions = [], savedPostIds, followingIds, currentUserId, onCategoryChange, onPrevious, onNext, onShuffle, onVote, onShare, onToggleSave, onToggleFollow, onBoost, onStartUpload, onAddComment }) {
  ...
  return <section className="editorial-feed relative flex h-full w-full min-h-0 flex-col items-center">
    ...
    <article onPointerDown={startCardGesture} onPointerMove={moveCardGesture} onPointerUp={finishCardGesture} onPointerCancel={() => { gestureStart.current = null; setIsDraggingMedia(false); setDragOffset(0); }} onWheel={moveCardByWheel} className={`relative z-10 h-full min-h-0 w-full touch-none overflow-hidden rounded-xl border border-surface-container-high/60 bg-[#fbfaf7] shadow-2xl ${feedMotion}`}>
      ...
      <div className="feed-top-overlay"><div className="feed-top-overlay__row"><div className="flex items-center gap-1 rounded-full border border-white/20 bg-black/35 px-2 py-0.5 shadow-lg backdrop-blur-sm"><span className="h-1.5 w-1.5 animate-pulse rounded-full" style={{ backgroundColor: theme.color, boxShadow: `0 0 8px ${theme.color}` }} /><span className="font-mono text-[8px] font-bold leading-none tracking-wide text-white">LIVE STREAM</span><span className="font-mono text-[8px] leading-none text-white/75">{card.timestamp}</span></div><UserBadge author={card.author} canFollow={Boolean(card.author) && !card.isMyUpload && card.authorId !== currentUserId} following={followingIds?.has(card.authorId ?? `sample:${String(card.author).trim().toLowerCase()}`)} onToggleFollow={() => onToggleFollow?.(card.authorId ?? `sample:${String(card.author).trim().toLowerCase()}`)} /></div><div className="feed-top-overlay__category"><CategoryBadge theme={theme} category={card.category} /></div></div>
...
function UserBadge({ author, canFollow = false, following = false, onToggleFollow }) { return <div className="flex items-center gap-1 rounded-full border border-white/20 bg-black/35 py-0.5 pl-1 pr-1 shadow-lg backdrop-blur-sm"><span className="flex h-4 w-4 items-center justify-center rounded-full bg-primary-container/80"><span className="material-symbols-outlined text-[11px] text-white">person</span></span><span className="font-mono text-[8px] font-semibold leading-none tracking-wide text-white">@{author}</span>{canFollow && <button type="button" onClick={onToggleFollow} aria-pressed={following} aria-label={following ? `@${author} 팔로우 취소` : `@${author} 팔로우`} title={following ? '팔로우 취소' : '팔로우'} className={`ml-0.5 flex h-4 w-4 items-center justify-center rounded-full border border-white/35 bg-white/12 active:scale-90 ${following ? 'text-[#fff6cc]' : 'text-white'}`}>{following ? <UserCheck size={10} strokeWidth={2.4} /> : <UserPlus size={10} strokeWidth={2.4} />}</button>}</div>; }
