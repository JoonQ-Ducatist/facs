...
export default function FeedView({ locale = 'ko', categories, cards, card, currentIndex, activeCategory, hasVoted, isOwnPost = false, canViewLiveReactions = false, liveReactions = [], savedPostIds, followingIds, currentUserId, onCategoryChange, onPrevious, onNext, onShuffle, onVote, onShare, onToggleSave, onToggleFollow, onBlockAuthor, onBoost, onStartUpload, onAddComment }) {
...
function UserBadge({ author, canFollow = false, following = false, onToggleFollow, onBlock }) { const [menuOpen, setMenuOpen] = useState(false); const block = () => { if (window.confirm(`@${author} 계정을 차단할까요?\n서로의 피드와 팔로우 관계가 즉시 숨겨집니다.`)) onBlock?.(); setMenuOpen(false); }; return <div className="relative flex items-center gap-1 rounded-full border border-white/20 bg-black/35 py-0.5 pl-1 pr-1 shadow-lg backdrop-blur-sm"><span className="flex h-4 w-4 items-center justify-center rounded-full bg-primary-container/80"><span className="material-symbols-outlined text-[11px] text-white">person</span></span><span className="font-mono text-[8px] font-semibold leading-none tracking-wide text-white">@{author}</span>{canFollow && <><button type="button" onClick={onToggleFollow} aria-pressed={following} aria-label={following ? `@${author} 팔로우 취소` : `@${author} 팔로우`} title={following ? '팔로우 취소' : '팔로우'} className={`ml-0.5 flex h-4 w-4 items-center justify-center rounded-full border active:scale-90 ${following ? 'border-[#fff4bc] bg-black/50 text-[#fffde8] shadow-[0_0_7px_rgba(255,244,188,0.72)]' : 'border-white/55 bg-black/40 text-white shadow-[0_0_5px_rgba(255,255,255,0.35)]'}`}>{following ? <UserCheck size={10} strokeWidth={3} /> : <UserPlus size={10} strokeWidth={2.6} />}</button><button type="button" onClick={() => setMenuOpen((open) => !open)} aria-expanded={menuOpen} aria-label={`@${author} 메뉴`} className="flex h-4 w-4 items-center justify-center text-white/90"><span className="material-symbols-outlined text-[13px]">more_horiz</span></button>{menuOpen && <div className="absolute right-0 top-6 z-50 min-w-28 rounded-md border border-white/20 bg-[#101b2b]/95 p-1 shadow-xl backdrop-blur"><button type="button" onClick={block} className="w-full rounded px-2 py-1.5 text-left text-[10px] font-bold text-[#ffb1c4] hover:bg-white/10">차단</button></div>}</>}</div>; }
...
function Result({ yesPercent, noPercent, total, color, onNext, onBoost, onStartUpload, uploadLabel }) {
  ...
  if (sampleStatus === SAMPLE_STATUS.INSUFFICIENT) return <ResultShell total={total} color={color} onNext={onNext} onStartUpload={onStartUpload} uploadLabel={uploadLabel}><p className="text-[11px] leading-relaxed text-white/80">아직 충분한 첫인상이 모이지 않았어요. 10명의 반응이 모이면 첫 경향을 알려드릴게요.</p></ResultShell>;
...
function ResultShell({ total, color, sampleStatus, onBoost, onStartUpload, uploadLabel, onNext, children }) {
  ...
  return <div className="feed-result mb-2.5 rounded-xl border border-white/30 bg-[#061225]/42 p-2.5 shadow-[0_8px_24px_rgba(0,0,0,.18)] backdrop-blur-[1px]"><div className="feed-result__meta mb-1.5 flex items-center justify-between gap-2 pr-11 sm:pr-0"><span className="font-mono text-[10px] font-bold" style={{ color }}>{label}</span><span className="whitespace-nowrap font-mono text-[10px] text-white/80">유효 평가 {total.toLocaleString()}명</span></div>{children}<div className="feed-result__actions mt-1.5 flex items-center justify-between gap-2 pr-11 sm:pr-0"><div className="feed-result__secondary flex items-center gap-2">{onBoost && <button type="button" onClick={onBoost} className="feed-result__boost rounded-md border px-1.5 py-0.5 text-[10px] font-bold" style={{ color, borderColor: `${color}aa`, backgroundColor: `${color}20` }}>100명까지 Boost · ₩1,000</button>}{onStartUpload && <button type="button" onClick={onStartUpload} className="feed-result__self text-[10px] font-semibold text-white/85 underline decoration-white/35 underline-offset-2">{uploadLabel ?? '나도 평가받기'}</button>}</div><button type="button" onClick={onNext} className="feed-result__next flex items-center text-[11px] font-bold" style={{ color }}>다음 사진 <span className="material-symbols-outlined text-[13px]">arrow_downward</span></button></div></div>;
}
