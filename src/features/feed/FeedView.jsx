...
function CardMedia({ card, media, className, muted = false }) { const source = media ?? { type: card.mediaType ?? 'image', url: card.imageUrl, objectPosition: card.objectPosition }; const isVideo = source.type === 'video' || String(source.type ?? '').startsWith('video/'); const videoPoster = useVideoPoster(isVideo ? source.url : ''); const protectMedia = (event) => event.preventDefault(); const isolateVideoTouch = (event) => event.stopPropagation(); return isVideo ? <video className={className} style={{ objectPosition: source.objectPosition ?? card.objectPosition }} src={source.url} poster={videoPoster || undefined} autoPlay={Boolean(muted)} loop={Boolean(muted)} muted={muted || undefined} playsInline preload="metadata" controls={!muted} draggable="false" onPointerDown={isolateVideoTouch} onPointerMove={isolateVideoTouch} onPointerUp={isolateVideoTouch} onPointerCancel={isolateVideoTouch} onContextMenu={protectMedia} onDragStart={protectMedia} aria-label={`${card.author}의 ${card.category} 동영상`} /> : <img className={className} style={{ objectPosition: source.objectPosition ?? card.objectPosition }} src={source.url} alt={`${card.author}의 ${card.category} 사진`} draggable="false" onContextMenu={protectMedia} onDragStart={protectMedia} />; }
...
function Result({ yesPercent, noPercent, total, color, onBoost, onStartUpload, uploadLabel }) {
  ...
  if (total === 0) return <ResultShell total={total} color={color} onStartUpload={onStartUpload} uploadLabel={uploadLabel}><p className="text-[11px] leading-relaxed text-white/80">아직 평가에 참여한 사람이 없어요. 평가가 시작되면 결과를 알려드릴게요.</p></ResultShell>;
