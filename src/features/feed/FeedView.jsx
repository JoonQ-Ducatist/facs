...
function AgeVotePanel({ card, color, onVote }) {
  ...
  return <div className="age-vote-panel rounded-xl border border-white/30 bg-[#061225]/28 px-2.5 py-1.5 shadow-[0_4px_14px_rgba(0,0,0,.12)]"><p className="mb-1 text-center text-[10px] font-semibold leading-none text-white">몇 살로 보이나요?</p><div className="flex items-center gap-1.5"><button type="button" onClick={() => changeAge(selectedAge - 1)} disabled={selectedAge <= min} aria-label="예상 나이 한 살 낮추기" className="flex h-6 w-6 shrink-0 items-center justify-center rounded-full border border-white/35 text-white disabled:opacity-35"><span className="material-symbols-outlined text-[14px]">remove</span></button><span className="shrink-0 font-mono text-[10px] font-bold" style={{ color }}>{min}</span><input type="range" min={min} max={max} step="1" value={selectedAge} onChange={(event) => changeAge(event.target.value)} onPointerDown={(event) => event.stopPropagation()} onPointerMove={(event) => event.stopPropagation()} onPointerUp={(event) => event.stopPropagation()} aria-label="예상 나이 선택" aria-valuetext={`${selectedAge}세`} className="age-vote-slider min-w-0 flex-1" style={{ '--age-slider-color': color }} /><span className="shrink-0 font-mono text-[10px] font-bold" style={{ color }}>{max}</span><button type="button" onClick={() => changeAge(selectedAge + 1)} disabled={selectedAge >= max} aria-label="예상 나이 한 살 높이기" className="flex h-6 w-6 shrink-0 items-center justify-center rounded-full border border-white/35 text-white disabled:opacity-35"><span className="material-symbols-outlined text-[14px]">add</span></button><button type="button" onClick={() => onVote(selectedAge)} className="ml-0.5 shrink-0 rounded-md px-2 py-1 text-[10px] font-bold leading-none text-[#051424]" style={{ backgroundColor: color }}>선택</button></div><output className="mt-1 block pl-7 font-mono text-xs font-bold leading-none" style={{ color }}>{selectedAge}세</output></div>;
}
