...
export default function FeedView({ locale = 'ko', categories, cards, card, currentIndex, activeCategory, hasVoted, savedPostIds, onCategoryChange, onPrevious, onNext, onShuffle, onVote, onShare, onToggleSave, onBoost, onStartUpload, onAddComment }) {
  ...
  return <section className="editorial-feed relative flex h-full w-full min-h-0 flex-col items-center">
    ...
    <article onPointerDown={startCardGesture} onPointerMove={moveCardGesture} onPointerUp={finishCardGesture} onPointerCancel={() => { gestureStart.current = null; setIsDraggingMedia(false); setDragOffset(0); }} onWheel={moveCardByWheel} className={`relative z-10 h-full min-h-0 w-full touch-none overflow-hidden rounded-xl border border-surface-container-high/60 bg-[#fbfaf7] shadow-2xl ${feedMotion}`}>
      ...
      <div className="absolute right-3 top-1/2 z-30 flex -translate-y-1/2 flex-col gap-2"><ShareRailButton onClick={() => onShare(card)} /><RailSaveButton locale={locale} saved={isSaved} color={theme.color} notice={saveNotice} onToggle={toggleSavedCard} /><div className="flex flex-col gap-2.5"><ArrowButton label="이전 카드" icon="expand_less" onClick={() => navigateFeed(-1)} /><ArrowButton label="다음 카드" icon="expand_more" onClick={() => navigateFeed(1)} /></div></div>
...
function MediaProgress({ locale, media, mediaIndex, color, onSelect }) { return <div className="media-progress absolute bottom-3 left-1/2 z-30 flex -translate-x-1/2 items-center gap-1" aria-label={locale === 'en' ? `${media.length} uploaded photos` : `등록된 사진 ${media.length}장`}>{media.map((item, index) => <button key={item.id} type="button" aria-label={locale === 'en' ? `View photo ${index + 1}` : `${index + 1}번째 사진 보기`} aria-current={mediaIndex === index ? 'true' : undefined} onClick={() => onSelect(index)} className="rounded-full p-0"><span className="block rounded-full transition-colors" style={{ backgroundColor: mediaIndex === index ? color : 'rgba(255,255,255,.52)' }} /></button>)}</div>; }
/** 정의: 로그인 계정에만 저장되는 비공개 Scrap 제어다. */
function RailSaveButton({ locale, saved, color, notice, onToggle }) { const action = locale === 'en' ? (saved ? 'Remove from Scraps' : 'Save to Scraps') : (saved ? '스크랩에서 제거' : '스크랩에 저장'); return <div className="rail-save-control relative"><p role="status" aria-live="polite" className="sr-only">{notice}</p><button type="button" onClick={onToggle} aria-pressed={saved} aria-label={action} title={action} className="rail-save-control__button" style={saved ? { color, borderColor: `${color}cc`, backgroundColor: `${color}24` } : undefined}><Bookmark aria-hidden="true" size={19} strokeWidth={2.1} fill={saved ? 'currentColor' : 'none'} /></button></div>; }
