...
export async function signInWithLocalQaAccount(accountId) {
  ...
  let session;
  try {
    const response = await fetch('/__facs-qa/session', {
      method: 'POST',
    ...
    if (!response.ok || !payload?.access_token || !payload?.refresh_token) {
      return { ok: false, code: payload?.code ?? 'LOCAL_QA_SIGN_IN_FAILED' };
    }
    session = await supabase.auth.setSession({ access_token: payload.access_token, refresh_token: payload.refresh_token });
  } catch {
    return { ok: false, code: 'LOCAL_QA_SIGN_IN_FAILED' };
  }
  if (session.error || !session.data.user) return { ok: false, code: 'LOCAL_QA_SIGN_IN_FAILED' };
  return completeLocalQaSignIn(account, session.data.user, supabase);
}
