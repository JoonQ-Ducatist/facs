...
1. 해당 단계의 범위와 완료 기준을 이 문서와 `90-day-cash-loop-plan.md`에서 확인한다.
2. 구현과 자동 검증을 수행한다.
...
| UX-07 | 실제 등록 시각 표기 | 서버의 `published_at`을 기준으로 피드 상단에 상대 시간을 표시하고, 열려 있는 화면에서도 주기적으로 갱신 | 진행 중 — 실제 시간 계산·표시 연결 시작 |
...
| BE-01 | 인증·세션·역할 | 일반 사용자·운영자·관리자 권한이 분리됨 | 진행 중 — 이메일 매직링크 세션·로그인 유지 선택, 공개 `@아이디` 설정과 업로드 전 아이디 확인 UI를 연결. 실제 이메일 발송·복귀 E2E와 사용자 역할/RLS는 다음 단계 |
| BE-07 | 다중 인증 제공자 | Apple·Google·Kakao·이메일 가입·로그인, 계정 연결·해제, 제공자별 오류 처리를 지원 | 보류 — 2026-09-09 사용자 지시에 따라 Google·Kakao 실제 연결은 별도 요청 전까지 시작하지 않음. 스플래시에서는 이메일 인증만 제공 |
| BE-08 | 팔로우·차단 관계·가시성 인가 | 승인 없는 단방향 Follow, 양방향 노출을 차단하는 Block, Post visibility(public/followers) 권한 검사를 서버에서 처리 | 진행 중 — 차단 관계와 피드·투표·스크랩·집계 조회를 함께 제한하고, 단방향 Follow·followers 공개 인가를 서버에 연결. 사용자 차단·공개 범위 설정 화면은 후속 연결 |
...
| CNT-02 | 실제 피드·필터·페이지네이션 | published 콘텐츠만 조건에 맞게 노출 | 진행 중 — 팔로우한 작성자의 새 게시물 우선, 최근 평가 카테고리 기반 관심사 추천, 나머지 일일 랜덤 발견 순서를 서버에서 계산. 커서 페이지네이션과 피드 기준 설명은 후속 연결 |
| CNT-03 | 유효 투표·집계 | 카드당 1표, 중복 방지, 서버 집계 적용 | 진행 중 — DB 고유 제약, 서버 검증, 원시 투표 비공개, 현재 회원의 완료 상태 복구를 구현. 실계정 다중 사용자 E2E가 남음 |
| CNT-04 | 결과·신뢰도·표본 안내 | 오해 없는 결과 문구와 표본 상태 제공 | 진행 중 — 0~9/10~29/30~99/100+ 표본 상태와 Aggregate 전용 Result를 구현. 실제 표본 전환의 기기 간 검증이 남음 |
| CNT-05 | 신고·차단·검토 | 사용자 신고와 운영 처리 흐름 구현 | 진행 중 — 8개 신고 사유, 중복 신고 방지, 신고자·운영자 전용 접근과 차단 관계의 서버 권한 기반을 작성. 사용자 신고 진입점·운영 검토 화면은 후속 연결 |
...
| CNT-11 | 개인 계정 설정 | 프로필 사진·핸들, 로그인 수단, 알림, 기본 공개·댓글 설정, 차단 목록, 탈퇴·데이터 요청을 관리 | 진행 중 — 공개 `@아이디` 설정과 업로드·댓글 전 확인을 먼저 연결. 나머지 설정은 후속 단계 |
| CNT-12 | 실제 게시 시간 표기 | 서버의 UTC 게시 시각을 바탕으로 피드와 댓글 상세에 상대 시간을 표시하고, 화면에서는 추가 네트워크 요청 없이 경과 시간만 계산 | 진행 중 — 서버 게시 시각 연결과 한국어·영어 상대 시간 계산을 구현. 실제 기기 점검이 남음 |
