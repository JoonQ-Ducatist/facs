...
- Antigravity는 화면 노출 여부만으로 PASS하지 않는다. 업로드 차단·아이디 설정·탐색
  무결성·새로고침 복구를 필수 게이트로 확인하고, 목적지·안내 문구·다음 행동이 모두
  확인되지 않으면 BUG 또는 미검증으로 기록한다.
...
로그인, 평가 제출, 업로드, 결제 없이 탐색·관찰만 한다. 로그아웃 상태의 Upload 차단
목적지와 안내는 확인하지만, 인증된 아이디 저장·업로드·투표·새로고침 복구는 반드시
미검증으로 기록한다. 브라우저 도구가 불가하면 BLOCKED로 보고한다. 공개 화면 QA는 실제
인증·저장·집계 검증을 대신하지 않는다.
