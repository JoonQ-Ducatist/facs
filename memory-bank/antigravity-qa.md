...
- 화면·입력·탐색·인증·업로드·저장 흐름이 변경되면 변경 화면만 훑지 않는다. 재현 경로와 인접 흐름을 묶어 확인한다. 특히 모바일 입력 변경은 `키보드 열기 → 입력 → 키보드 닫기 → 제출 버튼 첫 탭 → 목적지`를 하나의 필수 게이트로 기록한다.
- 화면 높이·키보드·고정 메뉴·화면 전환 결함은 자동 브라우저 결과만으로 PASS 처리하지 않는다. iPhone Safari·Chrome, Android Chrome·Samsung Internet, PC Chrome·Safari·Edge 결과가 없으면 반드시 `실기기 미검증`으로 보고하며 운영 반영의 근거로 사용하지 않는다.
...
- 검증 URL을 Chrome 새 탭으로 여는 동작은 사용량 영향이 거의 없으므로 기본 제공한다. 다만
  반복적인 접근성 스냅샷·스크린샷·탭 재조회는 컨텍스트 사용량을 늘릴 수 있으므로, URL을
  먼저 열고 스냅샷은 실제 조작 안내나 결함 확인에 필요한 경우에만 최소 횟수로 호출한다.
