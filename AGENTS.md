...
4. **선택 검증:** 실행 전에 각 명령이 실제로 돌릴 개별 테스트 수를 세고, 변경 계약에
   직접 연결된 테스트 2~5개와 핵심 브라우저 흐름 1회만 실행한다. 한 파일에 여러 테스트가
   있으면 파일 단위 실행을 금지하고 이름 필터 또는 더 작은 계약 테스트로 좁힌다. 인증·권한·
   데이터 계약처럼 위험이 큰 변경은 정상·실패·복구 경로를 포함해 최대 8개까지 선정할 수
   있으며, 그보다 넓은 범위는 전체 점검과 동일하게 사용자 승인 후에만 실행한다. QA 데이터는
   식별 조건을 먼저 기록하고 과제 생성분만 정리한다.
